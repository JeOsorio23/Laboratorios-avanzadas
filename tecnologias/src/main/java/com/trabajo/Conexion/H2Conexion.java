...
public class H2Conexion implements Conexion {
    ...
    private static H2Conexion instancia;

    private H2Conexion() {
        try {
            Class.forName("org.h2.Driver");
            connection = DriverManager
                    .getConnection("jdbc:h2:~/cursos_inscritos;AUTO_SERVER=TRUE;DB_CLOSE_ON_EXIT=TRUE", "sa", "");
            inicializarTablas();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static H2Conexion getInstancia(){
        if(instancia == null){
            synchronized(H2Conexion.class){
                if(instancia == null){
                    instancia = new H2Conexion();
                }
            }
        }
        return instancia;
    ...
    private void inicializarTablas() {
        try (Statement statement = connection.createStatement()) {
            ...
            statement.execute("CREATE TABLE IF NOT EXISTS inscripcion(\r\n" +
                    ...
                    +
                    ")");
            // Apartir de aqui se debe quitar una vez funcione las inscripciones en la
            // interfaz
